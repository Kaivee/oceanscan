  ...
  return (
    <div className="border-t border-[var(--color-ocean-border)] bg-[var(--color-ocean-card)] px-2 pb-1 pt-1">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="block h-16 w-full sm:h-[70px]" preserveAspectRatio="none">
        ...
        <path d={lineD} fill="none" stroke="#8BE9FD" strokeWidth="1.4" opacity="0.85" />
          ...
          return (
            <g key={z.id}>
              <line
                ...
                x2={x}
                y2={H - PAD_B}
                stroke={active ? "rgba(255,184,108,0.8)" : "rgba(139,233,253,0.3)"}
                strokeWidth={active ? 1.4 : 1}
                strokeDasharray="3 3"
              />
              <text x={x + 3} y={PAD_T + 4} fontSize="7" fill={active ? "rgba(255,184,108,0.9)" : "rgba(139,233,253,0.65)"} fontFamily="monospace">
                {z.label}
              </text>
            </g>
          );
        ...
        {cursorX !== null && (
          <line x1={cursorX} y1={PAD_T} x2={cursorX} y2={H - PAD_B} stroke="rgba(255,184,108,0.9)" strokeWidth="1.2" />
        )}
      </svg>
    </div>
  );
